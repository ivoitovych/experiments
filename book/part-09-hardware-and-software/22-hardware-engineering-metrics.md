...
**Feedforward delay** is the latency between a measurement outcome and the application of a quantum gate conditioned on that outcome. Three components contribute: measurement latency, classical processing (decoding, decision logic), and pulse-generation latency. On the best-engineered superconducting devices (Google Willow, IBM Heron) the total round-trip is on the order of $100$–$500\\,\mathrm{ns}$ — fast enough to apply teleportation-style Pauli corrections within the coherence window. On ion traps the round-trip is dominated by measurement latency and is much slower, but the much longer coherence makes the *budget* in coherence-times still favourable.
