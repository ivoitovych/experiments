...
The catch lives in the **controlled exponentials** $\mathrm{C}U^{2^k}$. If $U$ has efficient closed-form powers — parameterised rotations, modular-exponentiation circuits, $e^{-iHt}$ via Trotter decomposition (Chapter 16) — then $U^{2^k}$ also has a $\mathrm{poly}(k)$ implementation by repeated squaring. If $U$ is a general black-box unitary with no such structure, $U^{2^k}$ generally requires $2^k$ applications of $U$, blowing up the gate count. This is why phase estimation is most useful when $U$ admits an efficient power structure (e.g., $U = e^{-iHt}$ via Trotterised simulation, Chapter 16) and not as a universal black-box subroutine. The required precision $t = O(\log(1/\epsilon))$ on the estimation register gives the standard $\epsilon^{-1}$-scaling in total controlled-$U$ applications — the same scaling amplitude estimation inherits (§14.8), and the quadratic improvement over the $\epsilon^{-2}$ of classical sampling.
