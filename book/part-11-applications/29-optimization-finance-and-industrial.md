...
**The size disconnect.** A 2026-era gate-model device offers roughly $10^2$ usable *physical* qubits (more raw qubits exist, but useful depth is limited by decoherence — these are noisy physical qubits, not error-corrected logical qubits in the Chapter 19 sense). A 2026-era D-Wave Advantage2 system offers about $7{,}000$ qubits but with restricted connectivity (Pegasus topology, degree 15). Real industrial QUBOs that practitioners want to solve are routinely $10^4$ to $10^7$ variables. The mismatch is structural, not just a matter of waiting for more qubits.
