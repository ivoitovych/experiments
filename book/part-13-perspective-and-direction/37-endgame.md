...
- **Part 2 (Chapters 4–6).** The mathematical and physical postulates. Chapter 5 is the load-bearing one: the four postulates of quantum mechanics in the form a programmer can compute against — state vectors in $\mathbb{C}^{2^n}$, unitary evolution, projective measurement, and tensor-product composition. Every later chapter is downstream of these four statements.
- **Parts 3–4 (Chapters 7–10).** Single-qubit and multi-qubit gates, circuit composition, and the universality of small gate sets. The point at which a reader could, in principle, implement and simulate any small circuit by hand.
- **Parts 5–7 (Chapters 11–16).** Algorithms. Chapter 14 (Shor's algorithm) and Chapter 15 (Grover's algorithm) are the two canonical exponential and quadratic speedups. Chapters 12 (QFT), 13 (phase estimation), and 16 (HHL and quantum linear algebra) fill in the algorithmic toolkit. Every "quantum advantage" claim outside of analog simulation traces back to a primitive in this part.
- **Part 8 (Chapters 17–19).** Variational algorithms (VQE, QAOA), error correction (stabiliser codes, surface code), and the threshold theorem. The bridge between the noiseless theoretical model and any device that could conceivably run a large algorithm.
- **Part 9 (Chapters 20–24).** Hardware platforms (superconducting, trapped-ion, neutral-atom, photonic), control stacks, compilation, and classical simulation. Chapter 20 in particular grounds the abstract qubit in real physical degrees of freedom — without which everything in Parts 5–8 floats free.
- **Part 10 (Chapters 25–27).** Practice and era. Chapter 25 is the NISQ-and-early-FT honest accounting that this chapter builds on directly. Chapter 26 is the practical access to current hardware. Chapter 27 is post-quantum cryptography, the one application where action is required *now*, before any quantum computer exists at the relevant scale.
- **Parts 11–13 (Chapters 28–37).** Applications (chemistry, optimisation, machine learning, finance), conceptual deepening, and the perspective material that culminates here.
...
- **Compiler engineer.** Mapping logical circuits to physical hardware: routing (Chapter 22), pulse-level optimisation, error-aware scheduling, fault-tolerant compilation. Compiler-engineering experience from classical (LLVM, MLIR, GPU shader compilers) transfers more directly than one would expect. The hardest part is the cost model — what counts as expensive on a quantum device is different from what counts as expensive classically.
