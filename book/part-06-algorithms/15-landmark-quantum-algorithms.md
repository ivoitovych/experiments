...
Grover's algorithm solves **unstructured search**: given an oracle $O_f$ for a boolean function $f: \\{0,1\\}^n \to \\{0,1\\}$ with $M$ marked inputs out of $N = 2^n$ total, find a marked input. The mechanism first, in one sentence: each iteration phase-flips the marked state (the oracle's only job) and then reflects every amplitude about the mean (the diffusion operator), so a little amplitude drains from the unmarked states into the marked one on every pass. The full algorithm wraps the amplitude amplification machinery of §14.7: initialise to the uniform superposition $|\psi\rangle = H^{\otimes n}|0^n\rangle$, apply the Grover operator $G$ a total of $k$ times — the near-optimal choice is $k = \mathrm{round}(\pi/(4\theta) - 1/2)$, often abbreviated to $\lfloor \pi/(4\theta) \rfloor$ in the small-$\theta$ limit, with $\sin\theta = \sqrt{M/N}$ — then measure. After $k$ iterations the probability of measuring a marked item is *exactly* $\sin^2\bigl((2k+1)\theta\bigr)$. With the rounded choice $k = \mathrm{round}\bigl(\pi/(4\theta) - 1/2\bigr)$ the angle $(2k+1)\theta$ lands within $\theta$ of $\pi/2$, so the success probability is at least $\cos^2\theta = 1 - M/N$; the query count is $\Theta(\sqrt{N/M})$. (The small-$\theta$ floor variant $k = \lfloor \pi/(4\theta) \rfloor$ gives a comparable but slightly weaker guarantee.)
...
Two extensions are worth knowing. **Unknown $M$**: the Boyer–Brassard–Høyer–Tapp strategy runs Grover with iteration counts drawn *at random* from geometrically growing ranges (randomisation defeats the periodic overshoot that deterministic doubling would hit), verifying each candidate against the oracle; expected total queries remain $O(\sqrt{N/M})$ without knowing $M$ in advance (and the $M = 0$ case is detected by repeated verification failure). **Quantum minimum-finding (Dürr–Høyer)**: $O(\sqrt{N})$ queries to find the minimum of $f$ over $\\{0,1\\}^n$, via iterated Grover with the current best as the marking threshold.
...
The implication, when (if) fault-tolerant quantum computers exist at the relevant scale, is that RSA and the discrete-log family of public-key cryptosystems are broken. This is the motivation for **post-quantum cryptography** (Chapter 27) and for the NIST standardisation effort that in August 2024 finalised its first three post-quantum standards — ML-KEM (FIPS 203) and ML-DSA (FIPS 204), derived from CRYSTALS-Kyber and CRYSTALS-Dilithium, and SLH-DSA (FIPS 205), derived from SPHINCS+.
...
The verdict: HHL is the prototype for "block-encoded linear algebra" and its successors (Chapter 16), but several headline HHL applications — recommendation systems, PCA, some kernel and regression proposals — have been *dequantised* by Tang and follow-ups, which construct classical algorithms with comparable polylogarithmic dependence under matching sample-and-query access assumptions (proposals under other access models remain unsettled).
...
VQE's appeal is that the ansatz can be tailored to the chemistry: **unitary coupled cluster (UCCSD)** uses excitation operators from coupled-cluster theory; **ADAPT-VQE** grows the ansatz one operator at a time based on largest gradient; **hardware-efficient** ansatze ignore chemistry and just minimise depth. UCCSD handles dynamic correlation well but — built on a single reference — is often poor for strong *static* correlation, and has deep circuits; ADAPT-VQE can reach similar accuracy with fewer gates on some systems, at the cost of many extra gradient measurements and classical orchestration.
...
with $2p$ real parameters. Classical optimisation tunes $(\vec\gamma, \vec\beta)$ to maximise $\langle H_C\rangle$ (with $H_C$ scaled so that better cuts mean larger eigenvalues — the extremal state is the *top* eigenstate under this sign convention); sampling from the optimised state then yields candidate solutions.

For $p = 1$, QAOA on Max-Cut on *triangle-free* 3-regular graphs achieves an approximation ratio of $0.6924\ldots$, beating naive random assignment. As $p \to \infty$, with a parameter schedule that interpolates adiabatically slowly, QAOA provably converges to the extremal state of $H_C$ (via the adiabatic theorem, given a nonclosing gap along the path — and note that the adiabatic schedule existing is not the same as a classical optimiser finding it); arbitrary optimised finite-$p$ schedules carry no such guarantee. The question is the rate of convergence and whether the optimal $\vec\gamma, \vec\beta$ are efficiently learnable.
...
- **Speedups for classical ML via HHL-style algorithms** (linear regression, kernel methods, recommendation systems, principal-component analysis): the headline instances (recommendation systems, PCA, low-rank regression and kernel proposals) dequantised by Tang and follow-ups under matching sample-and-query access assumptions. Strong state-preparation/QRAM assumptions were one culprit; conditioning, rank, precision, and output-model fine print did the rest — and access models outside the dequantised family remain open.
