...
The principle holds because, until the measurement is actually performed, the qubit being measured remains a quantum object that the controlled gate can act on; the marginal distribution at the end is the same whether the measurement happens before or after the controlled operation. Formally: $\mathrm{Meas} \circ \mathrm{C}(U) = \mathrm{C}(U) \circ \mathrm{Meas}$ in the sense of joint outcome statistics.
...
A **mid-circuit measurement** is a measurement performed before the circuit ends, with the qubit available for further use afterwards. Two regimes matter.
...
Three canonical uses. **Teleportation-based gates**: the $T$ gate on a logical qubit, in many surface-code schemes, is implemented by consuming a magic state and applying a measurement-conditioned Clifford correction (an $S$ gate) — not merely a Pauli. **Error correction**: every cycle of a stabiliser code (Part 8) measures syndromes and applies corrections based on the syndrome bits; the correction layer is feedforward, often with a classical decoder in between. **Adaptive protocols**: measurement-based computation and many photonic schemes use feedforward to route between alternative branches of the computation.
