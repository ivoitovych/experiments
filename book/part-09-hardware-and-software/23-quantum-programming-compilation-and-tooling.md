...
**Dynamical decoupling (DD).** Insert sequences of $X$ or $XY4$ pulses into idle slots so that low-frequency noise averages out. The pattern is structurally identical to NMR refocusing pulses (and has the same theoretical justification: a fast drive averages the slow noise away before it can accumulate — first-order cancellation in the perturbation series control theorists call the Magnus expansion). Both Qiskit (`PadDynamicalDecoupling`) and tket apply DD automatically at high optimisation levels.
