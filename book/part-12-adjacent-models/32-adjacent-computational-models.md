...
The promise of topological quantum computation is therefore real but distant. If a non-Abelian anyon platform is engineered, its qubits would be passively protected at the hardware level, dramatically reducing the overhead of fault-tolerance compared to gate-model schemes that rely on active error correction (Chapter 19). If no such platform materializes, the model remains a useful theoretical construct and a source of inspiration for active-correction codes (the surface code is itself an emulation of an Abelian-anyon system).
