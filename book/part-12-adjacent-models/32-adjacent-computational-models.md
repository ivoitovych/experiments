...
with the cubic dependence reduced to quadratic under tighter hypotheses. The implication is that AQC is efficient (polynomial time) for problems whose interpolating Hamiltonian has an inverse-polynomial minimum gap, and inefficient otherwise. Identifying when this gap closes is, in general, as hard as the original problem.
...
The practical caveats are heavy. The Hamiltonians produced by the universality construction are typically $k$-local for $k \ge 3$, and engineering arbitrary $k$-local interactions in hardware is hard. Most physical implementations either restrict to 2-local Hamiltonians (sacrificing the cleanest universality argument) or use perturbative gadgets to simulate $k$-local terms at the cost of larger gaps and longer runtimes. The next section covers what happens when those compromises pile up and the adiabatic guarantee is dropped.
...
The natural physical realisation is an optical mode (a beam of light in a fixed spatial-temporal envelope); $\hat q$ and $\hat p$ become the two quadratures of the electromagnetic field. A **coherent state** $|\alpha\rangle$ is the closest classical-light analogue; **squeezed states** sharpen $\hat q$ at the expense of $\hat p$ (or vice versa), and **Fock states** $|n\rangle$ have definite photon number $n$. The gate set splits cleanly into two layers:
...
where $U_S$ is the submatrix of $U$ defined by the output pattern $s$. Computing the **permanent** of a matrix is #P-hard, and Aaronson and Arkhipov showed that sampling from this distribution (even approximately, under reasonable conjectures) is hard for classical computers. Boson sampling is therefore evidence of quantum-classical separation in the sampling regime, even though it cannot solve decision problems or run general algorithms.
...
## 32.9 Probabilistic and Randomised Computing

A briefer contrast: **probabilistic** or **randomised** computing extends deterministic classical computation by giving the machine access to fair coin flips. The complexity class **BPP** ("bounded-error probabilistic polynomial time") captures the decision problems solvable in polynomial time by such a machine with bounded error. Many practically important algorithms — primality testing, polynomial identity testing, Monte Carlo integration, randomised graph algorithms — are most naturally stated in BPP.

The parallel with quantum computation is structural but limited. Both BPP and **BQP** ("bounded-error quantum polynomial time") run on machines whose intermediate states are distributions over basis states; the difference is what kind of distribution. In BPP the intermediate state is a probability distribution — non-negative reals summing to one — and operations are stochastic matrices. In BQP the state is an *amplitude* vector — complex numbers whose moduli squared sum to one — and operations are unitary matrices. Negative and complex amplitudes admit **destructive interference**: two computational paths leading to the same outcome can cancel, which is impossible for probabilities. The interference is the *source* of the quantum advantage, and BPP $\subseteq$ BQP follows directly by simulating a coin flip with a Hadamard followed by a measurement.
...
1. Give the interpolating Hamiltonian $H(s) = (1-s) H_0 + s H_1$ for an adiabatic algorithm that finds the ground state of a transverse-field Ising model. Identify the regime in which the spectral gap closes and explain what happens to the runtime bound.
2. A user submits a QUBO with 200 logical variables to a D-Wave device whose coupling graph has degree 15. Estimate, qualitatively, the number of physical qubits required after minor embedding, and name two failure modes you would expect to see in the returned samples.
3. Describe how a single qubit Hadamard followed by a $T$ gate would be implemented on a measurement-based quantum computer with a 2D cluster state. Identify which measurement outcomes feed forward into the next layer's basis choices.
