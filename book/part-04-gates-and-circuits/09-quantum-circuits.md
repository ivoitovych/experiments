...
Depth lower bounds are sometimes available. Preparing *some* $n$-qubit states from $|0^n\rangle$ — GHZ among them, and any state with correlations spanning the full register — requires depth $\Omega(n)$ on nearest-neighbor architectures, because linear connectivity forces information to propagate step by step (product states, by contrast, need depth $1$); on a fully-connected architecture, depth $O(\log n)$ suffices for some families like the GHZ state. Many practical algorithms compiled into a fault-tolerant Clifford+$T$ gate set pay a synthesis factor of $O(\log^{c}(1/\epsilon))$ discrete gates *per continuous rotation* (§8.11), inflating gate count everywhere and depth wherever those rotations sit on the critical path — schematically $O(\mathrm{poly}(n) \cdot \log^{c}(1/\epsilon))$ overall, with the two factors coming from different layers of the compilation. For NISQ circuits compiled directly to a hardware-native continuous gate set this discrete-synthesis $\epsilon$-dependence does not appear (calibration and pulse-level precision limits remain).
