...
**Lattice-based.** Security reduces to short-vector or learning-with-errors (LWE / Module-LWE / Ring-LWE) problems on integer lattices. The dominant family in the NIST output. Key sizes are moderate (kilobytes), operations are fast, and the schemes are versatile — KEM (key encapsulation mechanism, the post-quantum replacement for key exchange), signatures, identity-based encryption, fully-homomorphic encryption all have lattice constructions. The downside is that the security reductions tie scheme security to *worst-case* lattice problems by way of *average-case* variants, and the constants in those reductions are loose; the standardised parameter sets are calibrated against *direct* cryptanalysis rather than against the worst-case reduction. **CRYSTALS-Kyber** (KEM) and **CRYSTALS-Dilithium** (signatures) are the workhorses; **Falcon** (signatures) is a NTRU-lattice scheme with smaller signatures but harder constant-time implementation.
...
- **March 2025**: HQC selected as a code-based KEM standard for cryptographic diversity (NIST IR 8545), to be standardised as FIPS 207 (draft pending; final expected in 2027). Falcon enters draft as FN-DSA / FIPS 206 (submitted for approval August 2025; IPD released that autumn).
...
**Twin-field QKD** circumvents the PLOB bound without any quantum memory: both users send phase-encoded weak pulses to an *untrusted* midpoint that performs single-photon interference, so the signal crosses only half the channel and the key rate scales as $\sqrt{\eta}$ rather than $\eta$ (Lucamarini et al. 2018). Twin-field demonstrations have pushed direct fibre links past 500, 830, and (by 2023) 1000 km at low rates; §33.4 covers the protocol and the deployment picture in detail. It is the leading near-term route past the ~300 km ceiling where standard point-to-point BB84 collapses — the midpoint is untrusted, so none of the trusted-node compromise above applies.
