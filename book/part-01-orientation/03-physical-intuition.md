...
computation and looks at the *physics* that makes those objects
all but unavoidable for us. The goal is to build the kind of mental model a working
...
The chapter is **deliberately light on formulas**. Numerical claims
...
quantities which look continuous when measured in bulk can turn
out, at the microscopic level, to take only a discrete spectrum of
values — which quantities, and in which systems, is itself physics
that the end of this section pins down.
...
symmetrically placed above and below the original beam axis. The deflection registers the silver atom's magnetic moment —
dominated by its unpaired outermost electron — whose spin's
...
spin-$1/2$. No stable intermediate value is ever observed (real
beams and detectors blur, but the resolved outcomes are two).
Whatever the prior history of the atom, the $z$-component of its spin reads out
...
in unbounded space have continuous spectra. Electrons bound in
atoms, photon *number* in an optical mode, the component of a spin
along any chosen axis — these are among the contexts in which
quantization appears, and they are precisely the contexts in which
we build qubits (a mode's field quadratures, by contrast, stay
continuous).
...
sum to one. Each amplitude carries both a magnitude and a phase; a measurement
in the amplitude's own basis never sees the phase, but operations
that recombine components — or a measurement in a phase-sensitive
basis — turn relative phase into observable statistics.
...
- The wavefunction is a **bookkeeping device for amplitudes**;
  probabilities are computed from the amplitudes relative to a
  specified measurement. A state with amplitudes $(\alpha, \beta)$
  and a state with amplitudes $(e^{i\theta}\alpha, \beta)$ assign the
  same probabilities to a measurement in the natural basis, and —
  when both amplitudes are nonzero — behave differently under
  operations that recombine the two components (purely diagonal
  operations leave the difference hidden).
- The amplitudes are complex, not real. The factor of $i$ is not
  ...
  amplitudes can be built — and can even simulate the complex theory
...
- The wavefunction is best not read as a material field in ordinary
  ...
  Chapter 35), even though for a single particle moving in three
...
polarization states are the basis; real-coefficient superpositions
are rotated linear polarizations; relative phases between the two
components give elliptical and circular polarizations; and the
natural measurement device — a polarizing beam splitter — implements
a projective measurement to excellent approximation (finite
extinction ratio, loss, and detector efficiency are the engineering
gap). Essentially every single-qubit textbook identity has a
polarization-photon translation,
...
The same interference physics is reproducible with **electrons**
(Tonomura and collaborators' biprism experiments, 1989), with **neutrons**, with **atoms**, with
...
every system tested under sufficiently coherence-preserving
conditions has shown it, and the tested mass scale keeps growing.
...
output. It is also not "the photon is a classical material wave that splits
its substance between the paths": the detectors at the output are
*clicks*, integer counts, and a single photon arrives at one
detector or the other, never half at each. (The *amplitude* does
propagate wave-like through both arms — what fails is reading it as
classical stuff.) The superposition is a
...
(Chapter 15) are all variations on this theme. These algorithms
and the Mach-Zehnder run on the same underlying fact: amplitudes
combine before they become probabilities.

A second observation: **interference requires indistinguishability**.
If at any point during the experiment a record forms of which path
the system took — even one no human looks at, held in an
environmental degree of freedom — the interference is suppressed:
completely if the record distinguishes the paths perfectly,
partially if partially (visibility tracks the overlap of the marker
states, and quantum-erasure experiments confirm the accounting). The double-slit pattern washes out into
...
- **Most observables have no fixed value before measurement**.
  ...
  degrees — a loose analogy (the classical vector still has definite
  components; the quantum outcome is genuinely undetermined). In
  operational quantum mechanics there is no preexisting answer; the
  measurement *produces* one.
...
- **The disturbance is not knowable in advance**. A spin in
  ...
  measurement *outcome*, not by anything the quantum state specifies
  in advance.
- **Back-action is not "the apparatus is clumsy"**. No apparatus,
  however refined, extracts full $x$-spin information without
  disturbing $z$-spin — weak measurements can trade less information
  for less disturbance, but the trade-off itself is set by the
  algebra of the observables, not by engineering.
...
\hbar/2$. For spin components the bound is state-dependent —
$\Delta S_x\\, \Delta S_z \ge (\hbar/2)\\,|\langle S_y \rangle|$, and the
right-hand side can vanish in particular states — so the honest
summary is not "always a positive bound" but "no state makes both
sharp".
...
accurately". You can measure either one as sharply as your apparatus
allows; what you cannot do is prepare a state in which *both* are simultaneously
...
not commute and cannot be jointly measured *sharply* (approximate
joint schemes exist, paying in precision). The cost of measuring
incompatible observables on the same state shows up in algorithms
like the variational quantum eigensolver (Chapter 15), where the
Hamiltonian to be estimated is typically decomposed into commuting
(or qubit-wise commuting) groups measured separately on freshly
prepared copies — one strategy among several (basis rotations,
classical shadows; §11.7).
...
and they power the protocols in this book that exploit correlations
beyond anything a local classical model can deliver. (This chapter's
states are pure; the mixed-state definition of entanglement — not
expressible as a mixture of product states — waits for Chapter 7.)
...
classical-looking — and perfect agreement in one fixed basis is, by
itself, classically easy (a shared coin flip reproduces it). What
defeats classical explanation is the next observation. The perfect correlation is not a
...
nonlinear optical crystal — beta barium borate ($\beta$-BBO) is a
standard choice, alongside periodically poled crystals — and with
low probability the pump excitation converts into two lower-energy
"signal" and "idler" photons whose polarizations,
...
Bell pairs at detected rates from hundreds of pairs per second per
milliwatt of pump power for the classic BBO arrangement to hundreds
of thousands for modern optimized sources (figures that depend on
bandwidth, collection, and detector efficiency). SPDC sources have
been the workhorse of most optical Bell-test experiments since the
mid-1990s.
...
settings on each side while the photons were in flight, too fast for
any light-speed signal to carry one side's setting to the other
(with the Prelude's caveat: the switching was periodic rather than
random).
...
empirical capstone: the main closable loopholes —
locality and detection efficiency — have been closed simultaneously
in single experiments, with freedom-of-choice pushed far back (not
closed in the same sense) by the cosmic-setting and human-choice
tests, and the results agree with quantum mechanics.
...
comes to behave, for measurements on the
system alone, like a *classical mixture* — a state where the system
is in $|\psi_1\rangle$ with probability $|\alpha|^2$ or in
$|\psi_2\rangle$ with probability $|\beta|^2$, with no remaining
interference between the two — exactly so in the limit of complete
dephasing; partial decoherence leaves partial interference. The information is not gone; it has
...
hyperfine clock states; neutral-atom qubits, long
protocol-dependent hyperfine/clock coherence under good vacuum;
photonic qubits, little dephasing in transit but finite loss and
mode mismatch; nitrogen-vacancy centers in diamond, electron-spin
coherence of milliseconds at room temperature and seconds at low
temperature (nearby nuclear spins last longer).

The dimensionless figure of merit is **the ratio of coherence time
to gate time** — a first-order proxy for how many operations fit in
the coherence window (the real budget is set as much by gate
infidelity, crosstalk, and leakage as by decoherence). Current superconducting hardware (as
...
(Chapter 19; the hardware side lives in Part IX) suppresses logical
error rates — stretching a logical qubit's effective coherence time
far past any individual physical qubit's is the visible payoff — by encoding logical information into entangled multi-qubit
...
do not track. Chapters 10 and 18 make this precise (channels and
Kraus operators in Chapter 10, the noise phenomenology in
Chapter 18): an open-system
...
the open-system generalization of unitary maps (Chapter 10).
...
Why? A large part of the standard answer is very fast decoherence
(the full story also needs pointer states, coarse-graining, and —
for "why this outcome" — the measurement questions of Chapter 35). A macroscopic object — a coffee cup, a
...
classical mechanics is usually an excellent approximation — a
rule-of-thumb direction, not a sufficient condition (superconductivity
is a macroscopic counterexample). Wherever the
...
is the *description* the formalism works with — whether it is also
the world's underlying furniture is an interpretation question
(Chapter 35) — and the apparent dichotomy is a property of the
questions one asks.
...
whose full pattern across incompatible measurements no local
classical model reproduces — settled empirically by the
loophole-free Bell tests of 2015.
Decoherence is the local image of unitary entanglement with an
inaccessible environment, and it is a dominant practical
constraint on existing quantum hardware (sharing the podium,
platform by platform, with loss, control error, and fabrication).
...
state space of a polarized photon, a spin-$1/2$ particle, the
lowest two levels of a transmon (a multilevel circuit used as an
...
2. A single photon is sent through a Mach-Zehnder interferometer
   ...
   Answer: each detector fires with probability $1/2$, assuming an
   ideal, fully distinguishing path marker — partial marking gives
   partial fringes. The which-path information destroys the
   interference.)
3. A polarization-entangled Bell pair (say the correlated state
   $|\Phi^+\rangle$) is shared between Alice and Bob, who are spatially separated. Alice measures her photon in
