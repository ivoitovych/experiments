...
- ✅ **Step 6 (this commit): D1 front-matter renumbering** — background/self-check sections renumbered §1.x → §SC.x (headings, `### SC.3.1`, all 14 sample-problem numbers, 6 internal refs); notation sections §2.x → §N.x (7 headings, 5 internal refs). Investigation resolved the one ambiguous external citation the old scheme had created: Chapter 10's "cf. §1.2" in fact targets Chapter 1 §1.2 (*What Quantum Computing Is and Is Not*), so it now resolves uniquely with no edit needed. No anchor links into either file existed. Lint's numbered-heading pattern extended to two-letter prefixes; PROGRESS/index regenerated; build selftest passes.
- ✅ **Step 7 (this commit): D8 generated TOC** — new `scripts/generate_toc.py` (wired as `make toc`) emits TOC.md from the delivered manuscript's actual headings (529 section entries with GitHub anchors); the original planned outline moved to `archive/plan-original-toc.md` with its reconciliation note intact. README's TOC.md description ("every section heading") is now true by construction.
- ✅ **Step 8 (this commit): D2 Phase semantics** — the audit found real drift: 21 of 48 status blocks carried Phase values disagreeing with `scripts/phases.py` (mostly the scaffold default "1"), silently mis-grouping PROGRESS.md. Resolution: `phases.py` declared authoritative; all 21 blocks synced; STYLE.md now documents Phase as historical writing-plan metadata (Status, not Phase, tracks revision state); a fifth lint rule checks Phase against `phases.py`; PROGRESS.md regenerated with correct groupings.
- ✅ **Step 9 (this commit): D6 figures, all five** — (1) BB84 case-analysis table added inline in §27.7 (code-span Unicode kets; GFM `\|` escapes; no math-in-table, per the Bug-5 policy); (2)–(5) four generated diagrams via new `figures-src/generate_diagrams.py` (matplotlib-only, kept separate from the Qiskit circuit generator, wired into `make figures`): filter functions (§18.3 — free evolution/Hahn/CPMG-4 computed exactly from the sign-switching integral), rotated d=3 surface-code lattice (§19.12 — stabiliser parities of both logical strings verified even against opposite-type checks), lattice-surgery merge/split (§19.22), and the repeater/entanglement-swapping chain (§33.3). All embedded with descriptive alt text; PNG previews eyeballed for correctness; STYLE.md's Diagrams section updated. This completes D1–D9 and the apply-list O1–O4+O7.
