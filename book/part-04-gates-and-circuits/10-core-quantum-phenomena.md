...
So a state with sharp $Z$ value (an eigenstate of $Z$, i.e. $|0\rangle$ or $|1\rangle$) has $\Delta Z = 0$. A direct calculation — using $X^2 = Y^2 = I$ and $\langle X\rangle = \langle Y\rangle = 0$ on $Z$-eigenstates — gives $\Delta X = \Delta Y = 1$: maximal uncertainty in the conjugate bases. (Robertson's relation splits by pair here: for the pairs involving $Z$ it is trivially satisfied — $\Delta Z = 0$ and the right-hand side also vanishes, e.g. $|\langle Y\rangle| = 0$ for the $Z$–$X$ pair — but for the $X$–$Y$ pair it is sharp, not vacuous: $\Delta X\\, \Delta Y \ge |\langle Z\rangle| = 1$, exactly saturated by $\Delta X = \Delta Y = 1$.) This is the formal version of the §6.6 mutually-unbiased-bases observation.

**Operational consequence for protocols.** Any procedure that gains information distinguishing *nonorthogonal* alternatives must disturb some states — quantitatively, more information means more disturbance (known eigenstates can be measured without disturbance, and weak measurements buy less disturbance with less information). Quantum key distribution (Chapter 27) builds a cryptographic primitive on this trade-off: an eavesdropper who measures BB84 photons in the wrong basis disturbs the state detectably in the residual bit-error rate — one ingredient of a security proof that also needs authentication, privacy amplification, and finite-key analysis. Syndrome measurement in error correction (Chapter 19) is engineered so that, ideally, it projects onto syndrome sectors while leaving the encoded logical information intact (faulty extraction can still propagate errors to data).
...
**Why this matters for protocol design.** Any candidate protocol that requires "signaling via entanglement" — using a remote measurement basis choice to encode a bit observable on the other end — is impossible. Teleportation (§7.12) needs classical communication precisely because of no-signaling — the quantum correlations alone do not carry the message — and needs specifically *two* bits because Alice must tell Bob which of the four Pauli corrections to apply. Superdense coding likewise requires Alice to physically send her qubit. The classical channel is not a redundancy: entanglement alone cannot carry a message — some physically transmitted carrier, classical bits in teleportation or the sent qubit in superdense coding, must actually travel.

## 10.7 Contextuality

**Contextuality** is the formal feature of quantum mechanics that goes beyond Bell-style nonlocality. It is *not* that quantum theory predicts different marginal statistics for $A$ depending on the compatible co-measurement — ideal compatible measurements leave marginals context-independent. The theorem is about *explanation*: no hidden-variable model can pre-assign outcome values to all observables in a way that is independent of the measurement context while respecting the algebra's functional relations.
...
Two features make Lindblad the canonical form. **Trace preservation**: $\mathrm{tr}(\dot\rho) = 0$, so $\rho$ remains a valid density matrix. **Complete positivity**: the induced map $\rho(0) \mapsto \rho(t) = e^{\mathcal{L} t}(\rho(0))$ is CPTP (§10.12). Lindblad evolution is the most general generator of a *Markovian* (time-homogeneous, CP-divisible semigroup) evolution consistent with both — non-Markovian continuous dynamics falls outside it.
...
2. **Completely positive** (CP): for any auxiliary system $R$, the extension $\mathcal{E} \otimes \mathrm{id}_R$ maps positive operators on $\mathcal{H}_A \otimes \mathcal{H}_R$ to positive operators on $\mathcal{H}_B \otimes \mathcal{H}_R$. Mere positivity is not enough — the *transpose* map is positive but not completely positive, which is exactly why the *partial* transpose (transpose applied to one subsystem) can produce negative eigenvalues on entangled states (the PPT criterion, §7.11) and why neither is a physical channel.
...
The completeness relation $\sum_i K_i^{\dagger} K_i = I$ enforces trace preservation; the sum-of-conjugations form enforces complete positivity. The minimum number of Kraus operators over all representations — the **Kraus rank** — is at most $\dim(\mathcal{H}_A) \cdot \dim(\mathcal{H}_B)$ (any particular representation may carry redundant extras).
