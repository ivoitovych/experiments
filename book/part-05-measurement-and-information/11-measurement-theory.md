...
The optimal measurement is the projective measurement onto the positive and negative eigenspaces of the operator $\pi_0\\, \rho_0 - \pi_1\\, \rho_1$. For orthogonal $\rho_0, \rho_1$ the trace norm $\\|\rho_0 - \rho_1\\|_1$ equals $2$ (trace distance $1$) and the discrimination succeeds with probability $1$; for identical states it is $0$ and no measurement helps — with equal priors the best strategy is a coin flip, and with unequal priors it is to always guess the more probable label (error $\min(\pi_0, \pi_1)$). The trace distance therefore acquires the operational meaning **"bias of the best discriminator"**, which Chapter 12 generalises further.
