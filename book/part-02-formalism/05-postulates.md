...
1. The postulates are the *idealized* picture — closed-system,
   ...
   POVMs in Chapter 11), and Chapter 18 catalogs the specific
   noise and decoherence processes of real hardware. This chapter
   introduces the extensions deliberately so the path from idealized
...
- **Superposition is a postulate, not a derived fact.** Because the
  ...
  normalization) a perfectly good state; when $|\psi_1\rangle$ and
  $|\psi_2\rangle$ are orthonormal, the normalization condition is
...
realize; quantum *circuits* are sequences and tensor products of them.
The taxonomy of common gates lives in Chapter 8 and Appendix B; the
fact that the model's closed dynamics are exactly the unitaries —
every allowed evolution is one, and the postulate forbids none of
them — is its content. (Whether given hardware can *reach* a
particular unitary is a separate question of controllability and gate
synthesis — Chapter 8.)
...
formulations of variational algorithms, and any time we discretize
...
Three things to internalize about this postulate:

- **Measurement is irreversible.** The state collapses to the
  projected ray; you cannot, in general, recover the pre-measurement
  superposition from the post-measurement system and the outcome
  alone (the interaction itself can be modeled as reversible unitary
  coupling to an apparatus, right up until the record is discarded —
  Chapter 10). This breaks Postulate 2's reversibility and is the
...
- **Measurement is intrinsically probabilistic.** Even with perfect
  hardware, repeated runs — each preparing the same state afresh and
  measuring the same observable — can yield different outcomes. (An
  immediate *re*-measurement of the same system after a projective
  outcome repeats that outcome.)
...
The projective form here is the idealized case. The more general
...
(its *generator*). Standard quantum gates can be written as unitaries generated by
Hermitian generators — an abstract logarithm, not necessarily the
Hamiltonian any hardware implements (Chapter 21) — for example, the
Pauli matrices (the $2 \times 2$ matrices $X, Y, Z$ of §4.5's sanity
checks, used as gates from Chapter 8 onward, with their
Bloch-rotation reading in Chapter 6) are themselves Hermitian observables,
and the rotation gates $R_x(\theta), R_y(\theta), R_z(\theta)$ (Chapter 6)
are the unitaries they generate via $e^{-i \theta P / 2}$, where $P$ is
...
$|\alpha|^2$ and 1 with probability $|\beta|^2$ is, as far as its
output distribution goes, fully described by those two weights — its
internals may hold more state, but the final marginal has no further
structure. A quantum
...
This is a central conceptual difference between the quantum state
space and the probability simplex. Note where the phase information
lives: a fixed-basis Born measurement discards *all* phase from its
own output probabilities; what preserves relative phase is the
*evolution before* measurement — unitaries convert relative phase
into amplitude differences that a later measurement can see. That
division of labor is what makes interference work.
...
- **Relative phase** is operationally meaningful. It can be
  *prepared* (by phase gates like $S$, $T$, $R_z$ — defined in
  Chapter 6), *manipulated* (by basis-changing gates like the
  Hadamard $H$ of §4.5), and *converted to amplitude
...
analyzed *up to global phase*. If your derivation produces a state
...
realization of $\rho$ as an ensemble; there are infinitely many
...
- **Maximally mixed state on $n$ qubits**: $\rho = I / 2^n$. Every
  ...
  mixed state carries no information about the system's preparation —
  though it still records the dimension, and the system may be
  entangled with a purifying system elsewhere.

The quantity $\mathrm{tr}(\rho^2) \in [1/d, 1]$ (in dimension $d$) is the **purity** of
...
Density matrices generalize both the measurement postulate and the
evolution postulate.
...
between $\rho$ and $O$, which is the operator-level home of expectation-value calculations
(for the projective observables of this chapter; generalized
measurements use $\mathrm{tr}(\rho E_m)$ — Chapter 11) regardless of
whether the state is pure or mixed. This is the unifying form: a
single line of operator algebra handles both ensembles and pure
states (though pure-state simulation still works with state vectors
directly, for cost reasons).
...
characterization of the partial trace: it is whatever map gives the
...
A direct operational consequence is **no-signaling**: if a
...
"trace out the environment" argument in noise modeling, every
...
- **Product states factor.** If $\rho_{AB} = \rho_A \otimes \rho_B$ is a product state then $\mathrm{tr}_B(\rho_{AB}) = \rho_A$ (using $\mathrm{tr}(\rho_B) = 1$). Product states have marginals you read off directly; and when a globally *pure* state is entangled, its marginals are necessarily mixed (for globally mixed states, a mixed marginal by itself proves nothing — Chapter 7).
...
These particular cross terms vanish under the partial trace because
their partner-subsystem factors ($|0\rangle\langle 1|$ and
$|1\rangle\langle 0|$) are traceless — not because partial traces
kill coherences in general; product-state coherences survive. Their
vanishing here is what makes the reduced state mixed despite the
joint state being pure.
...
- **No general state amplification.** You cannot turn a single qubit
  ...
  cannot be produced.
- **Quantum key distribution has a physical foundation.** An
  eavesdropper cannot silently copy quantum-encoded key bits, because
  the copying operation does not exist; turning that obstacle into a
  security *proof* takes disturbance bounds, protocol analysis, and
  authenticated classical channels (BB84, Chapter 27).
...
that). It only forbids a universal cloner that works on arbitrary unknown
inputs — and it forbids it in full generality: giving the would-be
cloner an arbitrary ancilla "machine" register does not rescue it
(the two-register proof above is the standard simplified form).
...
on a qubit is $5/6$ — strictly less than perfect — and per-copy
fidelity *decreases* toward the state-estimation limit as you ask for
more copies ($1 \to M$ cloning). This is the basis of
...
state, you cannot, by a closed-system unitary that leaves the second
register in a fixed blank state, reduce them to one copy — *delete*
the second. (Simply discarding a copy — tracing it out — is always
allowed; the theorem is about making the information vanish rather
than moving it.) The proof is structurally similar to
...
into the environment at a thermodynamic price (Landauer, Prelude
§0.9). The quantum novelty is the ban on copying and on unitary
blanking of *unknown* states, not information conservation per se.

Three consequences worth naming:

- **Reversible deletion only.** The closest analog of "delete a
...
- **Conservation of information** under closed-system evolution is why noise gets modeled via quantum channels (Chapter 10; the
  noise phenomenology is Chapter 18)
...
- **Quantum garbage collection is non-trivial.** In algorithm design,
  ancilla qubits used as scratch space must be returned to a known
  state before coherent reuse — typically by *uncomputation*, evolving
  them back with the inverse of the unitary that set them up;
  measurement-and-reset is the other standard exit. Ancillas left
  entangled with branch-distinguishing garbage change the main
  register's reduced state and can destroy the interference the
  algorithm needs. The
...
Chapter 6 specializes the postulates to a single qubit — the
