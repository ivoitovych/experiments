...
> **Status:** prereviewed · **Phase:** 3 · **Sections drafted:** 8 / 8
...
> **How to read this chapter.** Treat it as a reader's guide for Part 6, not as a results chapter. There are no theorems to prove and no circuits to memorize here. The three "primitives" in §§13.1, 13.2, and 13.4 — with the query model of §13.3 as their cost framework — and the resource-accounting vocabulary in §13.7 are the parts you will reuse. The complexity-theoretic framing in §13.7 can be skimmed on a first pass and revisited after Chapter 15 (Shor and Grover); the warnings about "quantum parallelism" in §13.1 and about "quantum-inspired" methods in §13.5 are worth absorbing before you read any popular account of the field.

## 13.1 Amplitude Manipulation

The slogan "a quantum computer tries all answers at once" is, taken literally, false in a way that misleads more than it teaches. A single application of a unitary $U$ to $H^{\otimes n}|0^n\rangle = \tfrac{1}{\sqrt{2^n}}\sum_x |x\rangle$ does evaluate $U$ "on every $x$" in the sense that each $|x\rangle$ acquires the amplitude $U_{yx}$ in the output. But measurement returns a single $y$, sampled from $|\sum_x U_{yx}/\sqrt{2^n}|^2$. No exponentially long list of separately readable evaluations ever exists — linear evolution transforms one state vector — and measurement reduces what does exist to a single bit string. A single quantum query gives the same *raw* per-shot information rate as a single classical query — at most $n$ bits, for matched oracle and output sizes — but as Deutsch–Jozsa and Bernstein–Vazirani show (§§14.2, 14.3), interference can pack a global feature of the function into those bits in a way no single classical query can.
...
**One query is one $U_f$.** Two queries means two applications of $U_f$, possibly with intervening unitaries. Applying $U_f$ to a superposition is still one query — the model charges per invocation, not per basis state. (Counting a *controlled* $U_f$ as one query is an additional modeling convention: for a truly unknown black box, coherent control is not constructible for free and must be granted explicitly, as must access to $U_f^\dagger$ when it differs from $U_f$.)
...
implementable, for $N = 2^n$, on $n$ qubits with $O(n^2)$ controlled rotations under an arbitrary-angle gate library (§14.5; general modulus $N$ and finite gate sets add implementation overhead). The QFT diagonalizes shift-invariant operations, just like the classical DFT, but it does so *in superposition*, which means a single application acts on the entire input register at once.
...
**Phase estimation.** Given a unitary $U$ with an eigenstate $|u\rangle$ and eigenphase $e^{2\pi i \varphi}$, the **quantum phase estimation** subroutine outputs an $m$-bit approximation of $\varphi$ using $m$ controlled-$U^{2^k}$ stages and an $O(m^2)$-gate inverse QFT. The payoff comes when the powers $U^{2^k}$ are cheap to implement — as in Shor's algorithm, where the classical constants $a^{2^k} \bmod N$ come from classical repeated squaring and each modular-multiplication circuit is synthesized directly, so no $2^k$-fold repetition of a base circuit ever happens — rather than by $2^k$ literal applications of a black-box $U$ (§14.6 develops this caveat). Phase estimation is the algorithmic engine of Shor's algorithm (the period $r$ is read off as a phase), of quantum-simulation eigenvalue extraction (Chapter 16), and of HHL-style linear-system solvers.
...
**The structure of $f$ determines what $H^{\otimes n}$ produces.** For constant $f$, the post-Hadamard amplitudes concentrate on $|0^n\rangle$. For balanced $f$, they avoid $|0^n\rangle$ entirely (Deutsch–Jozsa, §14.2). For an $f$ hiding a vector $s$ in $\mathbb{Z}_2^n$, each run yields a uniformly random $y$ satisfying $s \cdot y = 0$, and $O(n)$ runs accumulate enough linear equations to solve for $s$ (Simon, §14.4). The same circuit, with the same primitives, decodes different structure when fed different oracles.
...
**Verifiable output.** The algorithm outputs a candidate answer (a factor of $N$, a period $r$); a classical post-processing step verifies the answer in polynomial time. Wrong answers are detected and discarded; the algorithm is rerun until a correct answer surfaces. Shor's algorithm (Chapter 15) is the canonical example — phase estimation returns an approximation to a random multiple of $1/r$ (the phase $s/r$), the continued-fraction post-processing proposes a candidate $r$, and the candidate is verified directly by checking $a^r \equiv 1 \pmod N$ before use.

**Amplitude-amplification stopping.** Grover-style algorithms require *tuned* stopping: too few iterations leaves amplitude in the unmarked subspace, and *too many* overshoots and rotates amplitude back out. Integer iteration counts land only *near* the optimum for generic problem sizes; exact-success variants modify the reflection phases. Other variants (Grover with unknown number of solutions, fixed-point amplitude amplification) handle the overshoot risk in different ways.
...
with $\mathrm{BQP} \subseteq \mathrm{AWPP}$ also known. The relationships are mostly open and the beliefs are not uniform: $\mathrm{P} = \mathrm{BPP}$ is widely expected (derandomization), whereas $\mathrm{BPP} \subsetneq \mathrm{BQP}$ and $\mathrm{BQP} \subsetneq \mathrm{PSPACE}$ are believed but unproven. *No* strict separation among $\mathrm{BPP}$, $\mathrm{BQP}$, and $\mathrm{PSPACE}$ has been proved; the only nearby unconditional separation is the coarse $\mathrm{P} \subsetneq \mathrm{EXP}$ from the time hierarchy theorem (§17.1). Whether $\mathrm{BQP} = \mathrm{BPP}$ — whether quantum computers can be efficiently simulated by classical randomized computers in general — is open. The standard *belief* is no, supported by the candidate hard problems below; the standard *proof* is absent.
...
**Polynomial speedups.** Provable, ubiquitous, and based on amplitude amplification. Grover's algorithm gives a quadratic speedup for unstructured search; element distinctness, collision finding, and many graph problems inherit polynomial speedups from Grover-style subroutines or from the *quantum walk* framework. Quadratic speedups are robust but easily eroded: in fault-tolerant settings, the large constant and polylogarithmic overheads of error correction can swallow a factor-of-$\sqrt{n}$ improvement at every practically reachable size (a strictly constant factor cannot beat a growing asymptotic gain forever — the erosion is a finite-size fact), and one must compare the *fault-tolerant* end-to-end quantum running time (including error-correction overhead) to the *best practical* classical running time on the same problem instance to judge whether the speedup survives.
...
**Query complexity vs. time complexity vs. gate complexity vs. T-count.** These are four different cost metrics, and the answer to "what is the best quantum algorithm for $P$?" depends on which you mean. Query complexity counts oracle calls. Time complexity counts gates from a fixed universal set. Gate complexity may distinguish two-qubit gates (expensive) from single-qubit gates (cheap). **T-count**, in fault-tolerant settings, is the dominant cost because every $T$ gate consumes a magic state (§8.10). When reading a paper, identify which metric the authors are optimizing.

**"Quantum-inspired" and dequantization.** Some classical algorithms inspired by quantum techniques (notably tensor-network methods for many-body systems and Tang-style sampling algorithms for low-rank linear algebra) match or come close to claimed quantum speedups under realistic input-access models. A 2018-vintage "exponential speedup" can deflate to a polynomial one when the classical access model is corrected. This does not invalidate the quantum algorithms — it sharpens the question of where the *genuine* speedups live. In Part 6 we will be explicit about input-access assumptions and about which speedups survive scrutiny.

## 13.8 Bridge to Chapter 14

This chapter framed *how to think* about quantum algorithms; Chapter 14 starts on the concrete catalog. The four black-box algorithms there — Deutsch, Deutsch–Jozsa, Bernstein–Vazirani, and Simon — all run on the §13.5 template: prepare a uniform superposition with $H^{\otimes n}$, query the oracle, and apply $H^{\otimes n}$ again to convert the imprinted pattern into a measurement outcome — Deutsch, Deutsch–Jozsa, and Bernstein–Vazirani as literal single-query instances, Simon with an $n$-bit output register and $O(n)$ repetitions to collect linear equations. They give the smallest possible illustrations of phase kickback (§13.2), of oracle-query thinking (§13.3), and of hidden-structure extraction (§13.5). Simon's algorithm, in particular, is the algorithm Shor saw and generalized; reading it well makes Chapter 15 read itself.
