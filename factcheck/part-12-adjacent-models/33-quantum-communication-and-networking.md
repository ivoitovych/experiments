# Factcheck — §33 Quantum Communication and Networking

Mirrors `book/part-12-adjacent-models/33-quantum-communication-and-networking.md`.
Seeded from ledger Pass 4 (2026-05-28).

## §33.4 — Micius satellite QKD

- **Claim** (anchor): "launched in $2016$ and operational since $2017$, demonstrated decoy-state BB84"
- **Method**: external
- **Source**: EurekAlert/CAS press release (Bai–Zeilinger videoconference); Wikipedia (Quantum Experiments at Space Scale); *Science* 356, 1140 (2017)
- **Verified**: 2026-05 · **Verdict**: confirmed

## §33.4 — Micius entanglement distribution

- **Claim** (anchor): "distributed Bell-pair entanglement between two ground stations separated by $1200\,\mathrm{km}$"
- **Method**: external
- **Source**: Yin et al., *Science* (2017) — satellite-based entanglement distribution
- **Verified**: 2026-05 · **Verdict**: confirmed

## §33.4 — Twin-field QKD > 1000 km

- **Claim** (anchor): "more than $1000\,\mathrm{km}$ of installed and ultra-low-loss fibre"
- **Method**: external
- **Source**: Liu et al., "Experimental Twin-Field QKD Over 1000 km Fiber Distance", *Phys. Rev. Lett.* (2023), arXiv:2303.15795 (1,002 km record)
- **Verified**: 2026-05 · **Verdict**: confirmed

## §33.4 — TF-QKD demonstration distances

- **Claim** (anchor): "$511\,\mathrm{km}$, $605\,\mathrm{km}$, $658\,\mathrm{km}$, $830\,\mathrm{km}$"
- **Method**: external
- **Source**: series of TF-QKD experiments by USTC and Toshiba-Cambridge groups, 2019–2023
- **Verified**: 2026-05 · **Verdict**: confirmed

## §33.4 / §33.5 — 2024–2026 satellite-QKD missions

- **Claim** (anchor): SpeQtre / SpeQtral-1 (SG), QEYSSat (CA), EAGLE-1 (ESA/EU), QUBE-II, CAPSat
- **Method**: external
- **Source**: national space-agency / mission pages; The Quantum Insider
- **Verified**: 2026-05 · **Verdict**: confirmed (general statement)
- **Comment**: **open** — individual mission launch dates remain in flux; re-verify each revision.
