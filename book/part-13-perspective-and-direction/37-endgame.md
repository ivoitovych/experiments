...
- **Part 8 (Chapters 18–19).** Noise, error mitigation, and fault tolerance: the noise menagerie and pre-QEC mitigation (Chapter 18), then stabilizer codes, the surface code, magic-state distillation, and the threshold theorem (Chapter 19). The bridge between the noiseless theoretical model and any device that could conceivably run a large algorithm.
...
The calibration numbers worth carrying out of this book, summarized from Chapter 25 and updated through the most recent results at the time of writing:
...
- **Coherence.** Superconducting $T_1$, $T_2$ in the $100$–$300\\,\mu s$ range; trapped-ion coherence times in the seconds-to-minutes range, but with proportionally slower gates. The product of coherence time and gate rate — roughly, "gates per coherence time" — is one useful cross-platform ratio, not a platform-fair figure of merit: it omits gate fidelity, parallelism, connectivity, SPAM error, leakage, duty cycle, and correlated noise, and well-calibrated gates are often limited by control error rather than by coherence.
...
Two kinds of statement about the next decade can be extrapolated — under stated assumptions, with real uncertainty; the timing of specific events cannot. The extrapolations come from the trend lines fitted in §25.5 plus the surface-code arithmetic of §25.6.

**Extrapolable: trend lines.** Physical qubit counts grew roughly $1.5\times$ per year over $2018$–$2026$ — unevenly, platform by platform, and with advertised counts not always usable counts; two-qubit *error* fell by roughly half per platform generation, a looser trend still. Continuing the count trend from $\sim 10^3$ in 2026 gives $\sim 5{,}000$ per chip by 2030 — $\sim 10^4$ per *system* if several chips or traps are networked — at two-qubit error $\sim 10^{-3}$ if the error trend also holds: the regime where small distance-$d$ codes can suppress logical error below the constituent physical qubits (selected-operation demonstrations of this already exist). Carried to $2035$–$2040$, the same growth law gives $\sim 4 \times 10^4$ to $\sim 3 \times 10^5$ physical qubits; the often-quoted $10^5$–$10^6$ range requires the growth rate itself to accelerate. And whether any of this supports algorithms at $\sim 10^9$ logical operations depends on quantities the physical count alone does not fix — code distance, logical cycle time and error, magic-state factory capacity, and decoder throughput. The trend lines have been wrong before in both directions; treat the extrapolations as central scenarios, not predictions.

**Extrapolable: a rough arrival order.** Resource estimates across the literature tend to place applications in a rough order — by overall resource demand (logical qubits, non-Clifford counts, precision, data access), not physical-qubit count alone, and with real variation between studies:

- **Quantum chemistry** beyond classical reach — which means hard active spaces and strong-correlation regimes, not small molecular size as such (small molecules are usually classically tractable to high accuracy). Resource studies put interesting instances at $\sim 100$ logical *data* qubits, but not at modest depth: fault-tolerant phase-estimation chemistry (Chapter 28) demands very large non-Clifford gate counts even at that width. Still far cheaper than cryptographic Shor, and a plausible — not established — front-runner for a defensible *useful* advantage.
- **Quantum simulation of lattice models.** Trotterized Hubbard, spin-glass, and lattice-gauge-theory simulations (Chapter 28) sit in a similar range. The advantage is comparison against tensor-network and quantum-Monte-Carlo baselines rather than against classical brute force, which keeps the bar moving.
- **Optimization.** QAOA and quantum annealing for combinatorial problems (Chapter 29) are the most contested area; classical heuristics keep closing the gap on every benchmark. Optimization as a quantum-advantage application is plausible but not the front-runner.
- **Cryptographic break.** Shor on $2048$-bit RSA needs, in Gidney–Ekerå-style surface-code estimates, several thousand ($10^3$–$10^4$) logical qubits at code distance in the mid-$20$s, $\sim 10^9$–$10^{10}$ Toffoli-equivalent non-Clifford operations with magic-state distillation overhead, and $\sim 10^7$ physical qubits — the same scenario figures used in Chapters 27 and 36. Resource estimates put this in the $2035$–$2040$ window under aggressive trend-line assumptions; longer under conservative ones. This is the latest-arriving of the major applications.
...
The constructive framing for an engineer: build for the early-FT regime as the next *capability* milestone, treat the post-quantum migration as due well *before* any plausible break date — harvest-now-decrypt-later moves the deadline earlier than the milestone (Chapter 27, §27.6) — and discount any claim that places either at a single, confident year.
...
- Specific algorithms provide speedups of differing epistemic strength. Grover's quadratic speedup is *provable*, in the query model. Shor's is superpolynomial (commonly called exponential) over the best *known* classical factoring algorithm — no classical lower bound is proven. Phase estimation is a primitive whose advantage depends on the access model; HHL's exponential dimension dependence holds under restrictive sparsity, conditioning, state-preparation, and readout assumptions; suitably constructed Hamiltonian-simulation instances are $\mathrm{BQP}$-hard while many physically natural instances are classically tractable. Where the speedup is superpolynomial, the consequences if and when a device runs them at scale are dramatic. Where it is only polynomial (Grover), or where the exponential speedup is throttled by data-loading and readout (HHL on classical input), the practical advantage is far more contingent on constant factors and I/O overhead.
...
- The application landscape is *narrower* than marketing implies. Most computational problems are not in the small set where a quantum algorithm gives an asymptotic speedup. Sorting, hashing, most database operations, most everyday optimization, and the bulk of machine learning workloads do not have a known quantum speedup of practical interest. Quantum computers will not replace classical computers; they will sit alongside them as accelerators for specific tasks, much as GPUs do for matrix multiplication.
- Even within the application landscape that *does* have a known speedup, the constant factors and the I/O bottleneck (Chapter 16's "quantum RAM" caveat) often dominate. The asymptotic statement "Shor is superpolynomially faster than the best *known* classical factoring algorithm" is true as stated; an engineering statement like "a quantum machine attacks RSA-2048 more cheaply than a classical effort" is a contingent comparison of scenarios — machine cost, runtime, energy, and success probability against the general number field sieve on specified hardware — not something the asymptotics deliver on their own.
- Many companies described in press releases as "doing quantum computing" are doing classical optimization with quantum-inspired heuristics, or are running variational algorithms whose answers are reproducible on a laptop. The label "quantum" stretches; the substance under the label varies enormously (Chapter 36).
...
- **SDK developer.** Building and maintaining the open-source quantum SDKs (Qiskit, Cirq, PennyLane, OpenQASM tooling, Stim, the various pulse-level and compiler-level stacks). The work is recognizable software engineering — Python, C++, compiler internals, performance optimization — applied to a domain that needs a working knowledge of Chapters 8–11 and 21–22. The barrier to entry is the lowest in the field; the projects are open-source and contributions are visible.
- **Application researcher.** Writing the algorithms that will run on early-FT machines. Requires Chapters 14–18 and a specialization domain (chemistry, optimization, simulation). The work is half algorithm design, half resource estimation; the published output is paper-shaped. Most application researchers sit in academic groups or in the algorithm teams at quantum vendors.
...
- **Compiler engineer.** Mapping logical circuits to physical hardware: routing (Chapter 23), pulse-level optimization, error-aware scheduling, fault-tolerant compilation. Compiler-engineering experience from classical (LLVM, MLIR, GPU shader compilers) transfers more directly than one would expect. The hardest part is the cost model — what counts as expensive on a quantum device is different from what counts as expensive classically.
- **Classical-quantum integration engineer.** Building the systems that orchestrate hybrid workloads: a variational optimizer, a tensor-network preprocessor, a magic-state factory, a classical decoder running at the syndrome-extraction rate. This is full-stack engineering with low-latency requirements and a quantum component; it is increasingly its own role at the larger vendors.
- **Application domain expert.** Bringing a working knowledge of (say) drug discovery, materials science, finance, or logistics, and pairing with quantum-algorithm researchers to identify problem formulations where a quantum primitive could plausibly help. The expert does not need to write the circuits; they need to recognize which of their domain's problems are *shaped* like Hamiltonian simulation, ground-state estimation, or sampling. The hardest skill in the field, and the most undersupplied.
...
3. A vendor announces a $10{,}000$-physical-qubit chip with $99.9$ percent two-qubit fidelity. Using the surface-code arithmetic of §25.6 ($2 d^2 - 1$ physical qubits per logical patch at distance $d$), estimate the number of *idealized* logical patches at $d = 11$ — before routing space, ancillas, magic-state factories, spares, and yield shrink the usable number. Explain why the fidelity figure alone does not determine the logical error rate or whether $d = 11$ is the right distance. Then compare your patch count against a published RSA-2048 resource estimate (thousands of logical qubits plus factories, §15.3) rather than deriving a single number of your own.
...
5. State, in a single paragraph that you would defend to a skeptical classical-systems engineer, why quantum computing is worth studying *now* even though no production workload has a defended quantum advantage in 2026. The argument should not rely on any specific deployment timeline.
