...
For AES this is straightforward. AES-128 is widely deployed, AES-192 and AES-256 are available in every modern library, and migration is a configuration change. The recommendation since Bernstein et al. and reiterated by NIST is: *if you need post-quantum confidentiality against Grover, use AES-256, which retains 128 bits of effective security after the square-root speedup*. AES-128 is not catastrophically broken — $2^{64}$ quantum queries is still a substantial cost — but it sits below the post-quantum security floor that NIST sets at "category 1: at least as hard as AES-128 against a quantum attacker", and the cleanest answer is to move to AES-256 for any long-lived secret.
...
- **Falcon-512** (FN-DSA, draft FIPS 206, category 1): public key 897 B, signature 666 B. Smallest signatures of any standardised PQC signature scheme, but constant-time floating-point implementation is hard.
...
- **2024–2025**: HQC selected as a code-based KEM standard for diversity, finalised as FIPS draft. Falcon redrafted as FN-DSA / FIPS 206.
...
**For QKD.** Deploy only when (a) the link is point-to-point fibre under 100 km or a closed-geometry free-space link, (b) the threat model justifies the additional cost and operational complexity, and (c) the deployment is genuinely *layered with* PQC rather than replacing it. Common deployment patterns are intra-data-centre links between paired security facilities, undersea-cable trunk encryption between secure-facility endpoints, and inter-bank settlement links in jurisdictions that mandate it. QKD is *not* a substitute for PQC for general internet traffic; PQC and QKD address overlapping but different threat surfaces, and PQC is the universal solution and QKD is a niche augmentation.
